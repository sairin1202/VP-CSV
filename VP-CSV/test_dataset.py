...
class VideoFolderDataset(torch.utils.data.Dataset):
    def __init__(self, folder, counter = None, cache=None, min_len=4, mode='train', load_images=False, id_file='train_seen_unseen_ids.npy'):
        ...
        #     print(imname.decode())
        #     im = PIL.Image.open('../../StoryViz/data/'+imname.decode()).convert('RGB')
        #     im = self.sample_image(im)[0]
        #     im.save(f'res/test/{idx}.png')
...
class StoryDataset(torch.utils.data.Dataset):
    ...
    def init_mart_vocab(self):
        ...
        print('get vocab size: ',len(self.vocab))
    ...
    def __getitem__(self, item):
        ...
        for idx, v in enumerate(lists):
            ...
            if self.out_dir:
                ...
                if random.random() > 0.5:
                    im_index.append(self.mask['_'.join(img_id.split('/')[-2:]) + f'_{sample_idx}'])
                else:
                    rand_mask = [0]*64
                    for _ in range(32):
        ...
        data_item = {'images': image, 'text':text, 'description': des,
                'subtitle': subs, 'images_numpy':image_numpy, 'labels':labels, 'image_layout': torch.Tensor(im_index).long()}
...
class ImageClfDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.out_dir is not None:
            image = PIL.Image.open(os.path.join(self.out_dir, '%s.png' % item)).convert('RGB')
        else:
